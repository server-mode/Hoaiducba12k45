  ...
  return (
    <div className="member-card group relative parallelogram-shape cursor-pointer aspect-[2/3]" onClick={() => onClick?.(member)}>
      <div className="image-container absolute inset-0 z-[2]">
        <img data-src={member.avatar} alt={member.name} onError={handleError} className="absolute inset-0 w-full h-full object-cover group-hover:scale-105 transition-transform duration-500 ease-in-out lazy-image" loading="lazy" />
      </div>
